...
do
for proto in {ton2o,to_central_rb,to_full_central_rb,torb,tofullrb};
do
for gc in off;
do
for preallocation in {1,4,8,12,16,20};
do
LD_PRELOAD=/home/yingjun/jemalloc/lib/libjemalloc.so timeout 180 ./src/tpcc -w ${warehouse} -b ${core_cnt} -p ${proto} -g ${gc} -k 1 -e -q tuple -l20 -P${preallocation} -n -y1 > /dev/null;
