...
for skew in {0.2,0.8};
do
for proto in {occn2o,pccn2o,ton2o};
do
for gc in n2otxn;
do
for size in {2000,4000,8000,10000,20000,40000};
do
LD_PRELOAD=/home/yingjun/jemalloc/lib/libjemalloc.so timeout 240 ./src/ycsb -z ${skew} -u ${update} -b ${core_cnt} -p ${proto} -g ${gc} -o ${opt} -k ${size} -e -P20 -L20 > /dev/null;
