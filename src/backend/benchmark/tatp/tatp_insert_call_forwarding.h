...
extern double data_length_15;
extern configuration state;
extern int RUNNING_REF_THRESHOLD;

class InsertCallForwarding : public concurrency::TransactionQuery {
 public:
  InsertCallForwarding()
      : sub_index_scan_executor_(nullptr),
        spe_index_scan_executor_(nullptr),
        call_insert_executor_(nullptr),
        context_(nullptr),
  ...
  void SetContext(executor::ExecutorContext* context) {
    ...
    call_insert_executor_->SetContext(context);
  ...
  virtual TxnType GetTxnType() {
    return TXN_TYPE_INSERT_CALL_FORWARDING;
  };
 ...
 public:
  ...
  executor::InsertExecutor* call_insert_executor_;
