...
class Amalgamate : public concurrency::TransactionQuery {
 public:
  Amalgamate()
  ...
  virtual const std::vector<Value>& GetCompareKeys() const {
    return savings_update_index_scan_executor_->GetValues();
  }

  // Common method
  virtual TxnType GetTxnType() {
    return TXN_TYPE_AMALGAMATE;
  };

  virtual std::vector<uint64_t>& GetPrimaryKeysByint() { return primary_keys_; }
  virtual int GetPrimaryKey() { return custid_0_; }
  ...
  virtual void UpdateLogTableSingleRef(bool canonical __attribute__((unused))) {
    ...
    key = std::string("CUSTID") + "-" + std::to_string(custid_0_);
    for (int i = 0; i < 5; i++) {
      concurrency::TransactionScheduler::GetInstance().LogTableIncrease(key);
    }

    key = std::string("CUSTID") + "-" + std::to_string(custid_1_);
    for (int i = 0; i < 2; i++) {
      concurrency::TransactionScheduler::GetInstance().LogTableIncrease(key);
    }
  }
  ...
  virtual void UpdateRunTableSingleRef(int queue_no,
    ...
    // id 0
    //////////////////////////////////////////////////////////////////////
    std::string key = std::string("CUSTID") + "-" + std::to_string(custid_0_);
