...
//                         Peloton
//
// optimistic_opt_n2o_txn_manager.cpp
//
// Identification: src/backend/concurrency/optimistic_opt_n2o_txn_manager.cpp
//
// Copyright (c) 2015-16, Carnegie Mellon University Database Group
//
//===----------------------------------------------------------------------===//

#include "optimistic_opt_n2o_txn_manager.h"
...
OptimisticOptN2OTxnManager &OptimisticOptN2OTxnManager::GetInstance() {
  static OptimisticOptN2OTxnManager txn_manager;
  return txn_manager;
}
...
VisibilityType OptimisticOptN2OTxnManager::IsVisible(
...
bool OptimisticOptN2OTxnManager::IsOwner(
    const storage::TileGroupHeader *const tile_group_header,
    const oid_t &tuple_id) {
...
// transaction.
// this function is called by update/delete executors.
bool OptimisticOptN2OTxnManager::IsOwnable(
    const storage::TileGroupHeader *const tile_group_header,
    const oid_t &tuple_id) {
  auto tuple_txn_id = tile_group_header->GetTransactionId(tuple_id);
...
bool OptimisticOptN2OTxnManager::AcquireOwnership(
    const storage::TileGroupHeader *const tile_group_header,
...
void OptimisticOptN2OTxnManager::YieldOwnership(const oid_t &tile_group_id,
  const oid_t &tuple_id) {
...
bool OptimisticOptN2OTxnManager::PerformRead(const ItemPointer &location) {
  LOG_TRACE("PerformRead (%u, %u)\n", location.block, location.offset);
  current_txn->RecordRead(location);
  return true;
}

bool OptimisticOptN2OTxnManager::PerformInsert(const ItemPointer &location, ItemPointer *itemptr_ptr) {
...
void OptimisticOptN2OTxnManager::PerformUpdate(const ItemPointer &old_location,
...
void OptimisticOptN2OTxnManager::PerformUpdate(const ItemPointer &location) {
  oid_t tile_group_id = location.block;
  oid_t tuple_id = location.offset;
...
void OptimisticOptN2OTxnManager::PerformDelete(const ItemPointer &old_location,
...
void OptimisticOptN2OTxnManager::PerformDelete(const ItemPointer &location) {
  oid_t tile_group_id = location.block;
  oid_t tuple_id = location.offset;
...
Result OptimisticOptN2OTxnManager::CommitTransaction() {
...
Result OptimisticOptN2OTxnManager::AbortTransaction() {
