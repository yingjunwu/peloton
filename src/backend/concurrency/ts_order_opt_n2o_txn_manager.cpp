...
//
// Identification: src/backend/concurrency/ts_order_opt_n2o_txn_manager.cpp
//
...
Spinlock *TsOrderOptN2OTxnManager::GetSpinlockField(
...
void TsOrderOptN2OTxnManager::SetHeadPtr(
    const storage::TileGroupHeader *const tile_group_header, 
...
TsOrderOptN2OTxnManager &TsOrderOptN2OTxnManager::GetInstance() {
  static TsOrderOptN2OTxnManager txn_manager;
  return txn_manager;
}
...
bool TsOrderOptN2OTxnManager::IsOwner(
    const storage::TileGroupHeader *const tile_group_header,
    const oid_t &tuple_id) {
...
// transaction.
// this function is called by update/delete executors.
bool TsOrderOptN2OTxnManager::IsOwnable(
    const storage::TileGroupHeader *const tile_group_header,
    const oid_t &tuple_id) {
  auto tuple_txn_id = tile_group_header->GetTransactionId(tuple_id);
...
void TsOrderOptN2OTxnManager::YieldOwnership(const oid_t &tile_group_id,
  const oid_t &tuple_id) {
...
bool TsOrderOptN2OTxnManager::PerformRead(const ItemPointer &location) {
...
void TsOrderOptN2OTxnManager::InitInsertedTupleForRecovery(storage::TileGroupHeader *tg_header, oid_t tuple_slot, ItemPointer *itemptr_ptr) {
...
bool TsOrderOptN2OTxnManager::PerformInsert(const ItemPointer &location, ItemPointer *itemptr_ptr) {
  oid_t tile_group_id = location.block;
  oid_t tuple_id = location.offset;
...
void TsOrderOptN2OTxnManager::PerformDelete(const ItemPointer &old_location,
...
void TsOrderOptN2OTxnManager::PerformDelete(const ItemPointer &location) {
  oid_t tile_group_id = location.block;
  oid_t tuple_id = location.offset;
...
Result TsOrderOptN2OTxnManager::AbortTransaction() {
