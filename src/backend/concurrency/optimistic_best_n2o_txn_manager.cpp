...
//                         Peloton
//
// optimistic_best_n2o_txn_manager.cpp
//
// Identification: src/backend/concurrency/optimistic_best_n2o_txn_manager.cpp
//
// Copyright (c) 2015-16, Carnegie Mellon University Database Group
//
//===----------------------------------------------------------------------===//

#include "optimistic_best_n2o_txn_manager.h"
...
OptimisticBestN2OTxnManager &OptimisticBestN2OTxnManager::GetInstance() {
  static OptimisticBestN2OTxnManager txn_manager;
  return txn_manager;
}
...
VisibilityType OptimisticBestN2OTxnManager::IsVisible(
...
bool OptimisticBestN2OTxnManager::IsOwner(
    const storage::TileGroupHeader *const tile_group_header,
    const oid_t &tuple_id) {
...
// transaction.
// this function is called by update/delete executors.
bool OptimisticBestN2OTxnManager::IsOwnable(
    const storage::TileGroupHeader *const tile_group_header,
    const oid_t &tuple_id) {
  auto tuple_txn_id = tile_group_header->GetTransactionId(tuple_id);
...
bool OptimisticBestN2OTxnManager::AcquireOwnership(
    const storage::TileGroupHeader *const tile_group_header,
...
void OptimisticBestN2OTxnManager::YieldOwnership(const oid_t &tile_group_id,
  const oid_t &tuple_id) {
...
bool OptimisticBestN2OTxnManager::PerformRead(const ItemPointer &location) {
  LOG_TRACE("PerformRead (%u, %u)\n", location.block, location.offset);
  current_txn->RecordRead(location);
  return true;
}

bool OptimisticBestN2OTxnManager::PerformInsert(const ItemPointer &location, ItemPointer *itemptr_ptr) {
...
void OptimisticBestN2OTxnManager::PerformUpdate(const ItemPointer &old_location,
...
void OptimisticBestN2OTxnManager::PerformUpdate(const ItemPointer &location) {
  oid_t tile_group_id = location.block;
  oid_t tuple_id = location.offset;
...
void OptimisticBestN2OTxnManager::PerformDelete(const ItemPointer &old_location,
...
void OptimisticBestN2OTxnManager::PerformDelete(const ItemPointer &location) {
  oid_t tile_group_id = location.block;
  oid_t tuple_id = location.offset;
...
Result OptimisticBestN2OTxnManager::CommitTransaction() {
...
Result OptimisticBestN2OTxnManager::AbortTransaction() {
