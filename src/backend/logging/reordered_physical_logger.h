...
//                         Peloton
//
// reordered_physical_logger.h
//
// Identification: src/backend/logging/reordered_physical_logger.h
//
...
class ReorderedPhysicalLogger {

public:
  ReorderedPhysicalLogger(const size_t &logger_id, const std::string &log_dir) :
  ...
  void StartLogging() {
    is_running_ = true;
    logger_thread_.reset(new std::thread(&ReorderedPhysicalLogger::Run, this));
  }
