...
namespace logging {
  ...
  void PhysicalCheckpointManager::StartCheckpointing() {
    is_running_ = true;
    central_checkpoint_thread_.reset(new std::thread(&PhysicalCheckpointManager::Running, this));
  }

  void PhysicalCheckpointManager::StopCheckpointing() {
    is_running_ = false;
    central_checkpoint_thread_->join();
  }

  void PhysicalCheckpointManager::Running() {
  ...
  void PhysicalCheckpointManager::PerformCheckpoint(const cid_t &begin_cid) {
    ...
    for (size_t i = 0; i < checkpointer_count_; ++i) {
      checkpoint_threads[i].reset(new std::thread(&PhysicalCheckpointManager::PerformCheckpointThread, this, i, begin_cid, std::ref(database_structures)));
    }
  ...
  void PhysicalCheckpointManager::PerformCheckpointThread(const size_t &thread_id, const cid_t &begin_cid, const std::vector<std::vector<size_t>> &database_structures) {
  ...
  void PhysicalCheckpointManager::CheckpointTable(storage::DataTable *target_table, const size_t &tile_group_count, const size_t &thread_id, const cid_t &begin_cid, FileHandle &file_handle) {
  ...
  // Visibility check
  bool PhysicalCheckpointManager::IsVisible(const storage::TileGroupHeader *const tile_group_header, const oid_t &tuple_id, const cid_t &begin_cid) {
