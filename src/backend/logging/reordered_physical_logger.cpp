...
//                         Peloton
//
// reordered_physical_logger.cpp
//
// Identification: src/backend/logging/reordered_physical_logger.cpp
//
...
void ReorderedPhysicalLogger::RegisterWorker(WorkerContext *physical_worker_ctx) {
  worker_map_lock_.Lock();
  worker_map_[physical_worker_ctx->worker_id].reset(physical_worker_ctx);
  worker_map_lock_.Unlock();
}

void ReorderedPhysicalLogger::DeregisterWorker(WorkerContext *physical_worker_ctx) {
  worker_map_lock_.Lock();
  worker_map_.erase(physical_worker_ctx->worker_id);
  worker_map_lock_.Unlock();
}

void ReorderedPhysicalLogger::StartIndexRebulding(const size_t logger_count) {
  PL_ASSERT(recovery_threads_.size() != 0);
  recovery_threads_[0].reset(new std::thread(&ReorderedPhysicalLogger::RunIndexRebuildThread, this, logger_count));
}

void ReorderedPhysicalLogger::WaitForIndexRebuilding() {
  recovery_threads_[0]->join();
}

void ReorderedPhysicalLogger::StartRecoverDataTables(const size_t checkpoint_eid, const size_t persist_eid,
  ...
  for (size_t i = 0; i < recovery_thread_count; ++i) {
    ...
    recovery_threads_[i].reset(new std::thread(&ReorderedPhysicalLogger::RunRecoveryThread, this, i, checkpoint_eid, persist_eid));
  }
}

void ReorderedPhysicalLogger::WaitForRecovery() {
...
void ReorderedPhysicalLogger::GetSortedLogFileIdList(const size_t checkpoint_eid, const size_t persist_eid) {
  // Open the log dir
  struct dirent *file;
  DIR *dirp;
...
txn_id_t ReorderedPhysicalLogger::LockTuple(storage::TileGroupHeader *tg_header, oid_t tuple_offset) {
...
void ReorderedPhysicalLogger::UnlockTuple(storage::TileGroupHeader *tg_header, oid_t tuple_offset, txn_id_t new_txn_id) {
...
bool ReorderedPhysicalLogger::InstallTupleRecord(LogRecordType type, ItemPointer new_tuple_pos, ItemPointer old_tuple_pos,
...
bool ReorderedPhysicalLogger::ReplayLogFile(const size_t thread_id, FileHandle &file_handle, size_t checkpoint_eid, size_t persist_eid) {
...
void ReorderedPhysicalLogger::RunRecoveryThread(const size_t thread_id, const size_t checkpoint_eid, const size_t persist_eid) {
...
void ReorderedPhysicalLogger::RunIndexRebuildThread(const size_t logger_count) {
  auto &manager = catalog::Manager::GetInstance();
...
void ReorderedPhysicalLogger::RebuildIndexForTable(const size_t logger_count, storage::DataTable *table) {
...
void ReorderedPhysicalLogger::Run() {
...
void ReorderedPhysicalLogger::PersistEpochBegin(FileHandle &file_handle, const size_t epoch_id) {
...
void ReorderedPhysicalLogger::PersistEpochEnd(FileHandle &file_handle, const size_t epoch_id) {
...
void ReorderedPhysicalLogger::PersistLogBuffer(FileHandle &file_handle, std::unique_ptr<LogBuffer> log_buffer) {
