...
namespace peloton {
  namespace logging {
    ...
    void DepLogger::RegisterWorker(std::shared_ptr<DepWorkerContext> worker_ctx) {
      worker_map_lock_.Lock();
      worker_map_[worker_ctx->worker_id] = worker_ctx;
      worker_map_lock_.Unlock();
    }

    void DepLogger::DeregisterWorker(DepWorkerContext *worker_ctx) {
      worker_map_lock_.Lock();
      worker_map_.erase(worker_ctx->worker_id);
      worker_map_lock_.Unlock();
    }

    std::vector<int> DepLogger::GetSortedLogFileIdList() {
      // Open the log dir
      struct dirent *file;
      DIR *dirp;
    ...
    txn_id_t DepLogger::LockTuple(storage::TileGroupHeader *tg_header, oid_t tuple_offset) {
    ...
    void DepLogger::UnlockTuple(storage::TileGroupHeader *tg_header, oid_t tuple_offset, txn_id_t new_txn_id) {
    ...
    bool DepLogger::InstallTupleRecord(LogRecordType type, storage::Tuple *tuple, storage::DataTable *table, cid_t cur_cid) {
    ...
    bool DepLogger::ReplayLogFile(FileHandle &file_handle, size_t checkpoint_eid, size_t pepoch_eid) {
    ...
    void DepLogger::RunRecovery(size_t checkpoint_eid, size_t persist_eid) {
      ...
      while (true) {
        ...
        // Pull log records from workers per epoch buffer
        {
          ...
          for (auto &worker_entry : worker_map_) {
            ...
            size_t last_seen_eid = worker_ctx_ptr->last_seen_eid;
            ...
            size_t worker_current_eid = std::min(current_global_eid,
                                                 epoch_manager.GetRwTxnWorkerCurrentEid(worker_ctx_ptr->txn_worker_id));
            worker_current_eids.push_back(worker_current_eid);

            if (last_seen_eid == worker_current_eid) {
              // The worker makes no progress
              continue;
            }

            // Persist the worker's log buffer
            for (size_t epoch_id = last_seen_eid; epoch_id < worker_current_eid; ++epoch_id) {
            ...
            worker_ctx_ptr->last_seen_eid = worker_current_eid;
          }
          worker_map_lock_.Unlock();
        }
    ...
    void DepLogger::PersistEpochBegin(const size_t epoch_id) {
    ...
    void DepLogger::PersistEpochEnd(const size_t epoch_id) {
    ...
    void DepLogger::PersistLogBuffer(std::unique_ptr<LogBuffer> log_buffer) {
