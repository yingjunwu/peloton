...
namespace peloton {
  namespace logging {
    ...
    thread_local DepWorkerContext* tl_dep_worker_ctx = nullptr;
...
// this function is called by each worker thread.
    void DepLogManager::RegisterWorker() {
      PL_ASSERT(tl_dep_worker_ctx == nullptr);
      // shuffle worker to logger
      std::shared_ptr<DepWorkerContext> shared_ctx_ptr(new DepWorkerContext(worker_count_++));
      ...
      tl_dep_worker_ctx = shared_ctx_ptr.get();
...
// deregister worker threads.
    void DepLogManager::DeregisterWorker() {
      ...
      size_t logger_id = HashToLogger(tl_dep_worker_ctx->worker_id);

      loggers_[logger_id]->DeregisterWorker(tl_dep_worker_ctx);

      log_workers_lock_.Lock();
      log_workers_.erase(tl_dep_worker_ctx->worker_id);
      log_workers_lock_.Unlock();
    }

    void DepLogManager::WriteRecordToBuffer(LogRecord &record) {
      DepWorkerContext *ctx = tl_dep_worker_ctx;
      ...
      if (is_success == false) {
        ...
        buffer_ptr = RegisterNewBufferToEpoch(std::move((ctx->buffer_pool.GetBuffer(ctx->current_eid))));
        // Write it again
    ...
    void DepLogManager::StartTxn(concurrency::Transaction *txn) {
      ...
      DurabilityFactory::StartTxnTimer(txn_eid, tl_dep_worker_ctx);

      PL_ASSERT(tl_dep_worker_ctx->current_eid == INVALID_EPOCH_ID || tl_dep_worker_ctx->current_eid <= txn_eid);

      // Handle the epoch id
      if (tl_dep_worker_ctx->current_eid == INVALID_EPOCH_ID
          || tl_dep_worker_ctx->current_eid != txn_eid) {
      ...
      tl_dep_worker_ctx->current_cid = txn_cid;
    ...
    void DepLogManager::CommitCurrentTxn() {
      PL_ASSERT(tl_dep_worker_ctx);
      LogRecord record = LogRecordFactory::CreateTxnRecord(LOGRECORD_TYPE_TRANSACTION_COMMIT, tl_dep_worker_ctx->current_cid);
      WriteRecordToBuffer(record);
    }
    ...
    void DepLogManager::FinishPendingTxn() {
      ...
      DurabilityFactory::StopTimersByPepoch(glob_peid, tl_dep_worker_ctx);

      // Dependent commit
      uint64_t current_time_in_usec = 0;
      TimerType timer_type = DurabilityFactory::GetTimerType();

      auto itr = tl_dep_worker_ctx->pending_txn_timers.upper_bound(glob_peid);

      while (itr != tl_dep_worker_ctx->pending_txn_timers.end()) {
        ...
        if (per_epoch_status_[idx] == EPOCH_STAT_COMMITABLE) {
          ...
          for (uint64_t txn_start_us : itr->second) {
            ...
            tl_dep_worker_ctx->txn_summary.AddTxnLatReport(current_time_in_usec - txn_start_us, (timer_type == TIMER_DISTRIBUTION));
          }

          itr = tl_dep_worker_ctx->pending_txn_timers.erase(itr);
        } else {
          itr++;
        }
      }
