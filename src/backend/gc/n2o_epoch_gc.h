...
class N2OEpochGCManager : public GCManager {
  friend class N2OSnapshotGCManager;

  N2OEpochGCManager(int thread_count, bool stand_by)
    : is_running_(true),
  ...
  public:
    ...
    virtual void RecycleOldTupleSlot(const oid_t &table_id, const oid_t &tile_group_id,
      ...
      (void) epoch_id;
      RecycleOldTupleSlot(table_id, tile_group_id, tuple_id, current_epoch_garbage_context);
    }
    ...
    virtual void CreateGCContext(size_t eid) override {
      CreateGCContext(eid, current_epoch_garbage_context);
    }
  ...
  private:
    ...
    void CreateGCContext(size_t eid, EpochGarbageContext *& garbage_ctx);

    void RecycleOldTupleSlot(const oid_t &table_id, const oid_t &tile_group_id,
                                     const oid_t &tuple_id, EpochGarbageContext *& garbage_ctx);
    ...
    void AddGarbageCtxToRecycleMap(std::shared_ptr<EpochGarbageContext> gc_ctx);

    void AddTupleToRecycleMap(TupleMetadata &tuple_meta);
